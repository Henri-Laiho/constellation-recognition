...
def xrecons_grid(X,B,A, filter=True):
    """
...
def save_image(x,count=0,start=0,directory='image'):
    for t in range(start, T):
        img = xrecons_grid(x[t], B, A)
        plt.matshow(img, cmap=plt.cm.gray)
        imgname = directory + '/count_%d_%s_%d.png' % (count,'test', t)  # you can merge using imagemagick, i.e. convert -delay 10 -loop 0 *.png mnist.gif
        plt.savefig(imgname)
        plt.close()
        print(imgname)