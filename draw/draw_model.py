...
class DrawModel(nn.Module):
    ...
    def generate(self, batch_size=64, initial_state=None):
        self.batch_size = batch_size
        with torch.no_grad():
            ...
            initial_state = torch.zeros(self.batch_size, self.A * self.B) if initial_state is None else initial_state
            for t in range(self.T):
                c_prev = Variable(initial_state) if t == 0 else self.cs[t - 1]
                z = self.normalSample()
                h_dec, dec_state = self.decoder(z, (h_dec_prev, dec_state))
                self.cs[t] = c_prev + self.write(h_dec)
                h_dec_prev = h_dec
